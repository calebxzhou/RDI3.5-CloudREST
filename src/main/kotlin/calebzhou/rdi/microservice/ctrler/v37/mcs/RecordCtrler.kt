...
class RecordCtrler(var mapper: RecordMapper) {
    ...
    fun recordChat(@RequestParam pid: String, @RequestParam cont: String) {
        mapper.insertRecordChat(RecordChat(pid,cont,TimeUtils.now))
    }
    ...
    fun recordDeath(@RequestParam pid: String, @RequestParam src: String) {
        mapper.insertRecordDeath(RecordDeath(pid,src,TimeUtils.now))
    }
    ...
    fun recordLogin(@RequestParam pid: String, @RequestParam ip: String, @RequestParam geo: String) {
        mapper.insertRecordLogin(RecordLogin(pid,ip,geo,TimeUtils.now))
    }
    ...
    fun recordIdName(@RequestParam pid: String, name: String) {
            val idName = RecordIdName(null, pid, name, TimeUtils.now)
        if(!mapper.containsRecordIdName(idName))
            mapper.insertRecordIdName(idName)
    }
    ...
    fun recordLogout(@RequestParam pid: String) {
        mapper.insertRecordLogout(RecordLogout(pid,TimeUtils.now))
    }
    ...
    fun recordBlock(
        ...
        mapper.insertRecordBlock(RecordBlock(null,pid,act, bid, world, x, y, z, TimeUtils.now))
    }
